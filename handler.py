...
class RateLimiter:
    ...
    def wait_if_needed(self, user_id: int):
        now = time.time()
        if user_id in self.last_call_times:
            elapsed = now - self.last_call_times[user_id]
            if elapsed < self.min_interval:
                sleep_time = self.min_interval - elapsed
                logger.info("Rate limiting", user_id=user_id, sleep_seconds=sleep_time)
                time.sleep(sleep_time)
        self.last_call_times[user_id] = time.time()
        ...
        for attempt in range(max_retries):
            try:
                ...
                if 400 <= e.code < 500 and e.code != 429:
                    logger.error("Client error, not retrying", error=e, http_code=e.code, attempt=attempt+1)
                    raise
                
                if attempt < max_retries - 1:
                    logger.warning("API call failed, retrying", error=e, attempt=attempt+1, delay=delay)
                    time.sleep(delay)
                    delay *= 2
                else:
                    logger.error("Max retries reached", error=e, max_retries=max_retries)
            except Exception as e:
                last_exception = e
                if attempt < max_retries - 1:
                    logger.warning("Unexpected error, retrying", error=e, attempt=attempt+1, delay=delay)
                    time.sleep(delay)
                    delay *= 2
                else:
                    logger.error("Max retries reached", error=e, max_retries=max_retries)
...
def send_chat_action(chat_id: int, action: str = "typing"):
    ...
    return telegram_request("sendChatAction", payload)
...
def get_message_by_id(user_id: int, message_id: int):
    """Retrieve specific message from DynamoDB."""
    try:
        resp = table.get_item(
            Key={
                "user_id": str(user_id),
...
def save_conversation_message(user_id: int, role: str, content: str):
    ...
    timestamp = datetime.now(timezone.utc).isoformat()
    message_id = hashlib.md5(f"{user_id}{timestamp}{role}".encode()).hexdigest()[:12]
    
    item = {
        "user_id": str(user_id),
        "sort_key": f"conv#{timestamp}#{message_id}",
        "role": role,
        "content": content,
    ...
    try:
        table.put_item(Item=item)
        logger.info("Conversation message saved", user_id=user_id, role=role, outcome="success")
    except Exception as e:
        logger.error("Failed to save conversation message", error=e, user_id=user_id, role=role, outcome="failure")
...
def get_conversation_history(user_id: int, limit: int = None) -> list:
    ...
    try:
        ...
        logger.info("Conversation history retrieved", user_id=user_id, message_count=len(messages), outcome="success")
        return messages
    except Exception as e:
        logger.error("Failed to get conversation history", error=e, user_id=user_id, outcome="failure")
        return []
...
def clear_conversation_history(user_id: int) -> bool:
    ...
    try:
        ...
        logger.info("Conversation history cleared", user_id=user_id, deleted_count=len(items), outcome="success")
        return True
    except Exception as e:
        logger.error("Failed to clear conversation history", error=e, user_id=user_id, outcome="failure")
        return False
...
def read_note(user_id: int, note_id: str) -> dict:
    """READ operation - Retrieve a specific note"""
    try:
        resp = table.get_item(
            Key={
                "user_id": str(user_id),
...
@retry_with_backoff
def call_groq_api(messages: list, temperature: float = 0.7, max_tokens: int = 1024) -> dict:
    """Call Groq API with retry and backoff logic."""
    if not GROQ_API_KEY:
    ...
    logger.info("Calling Groq API", model=GROQ_MODEL, message_count=len(messages))
    ...
    logger.info("Groq API success", 
                ...
                usage_tokens=resp_data.get("usage", {}).get("total_tokens"),
                outcome="success")

    return resp_data

def chat_with_groq(user_id: int, user_message: str, use_history: bool = None) -> str:
    """Chat with Groq AI with optional conversation history."""
    if use_history is None:
        use_history = ENABLE_CONVERSATION_MEMORY

    rate_limiter.wait_if_needed(user_id)
    ...
    try:
        ...
        messages.append({
            ...
            "content": (
                ...
                "If asked about your capabilities, explain you can have natural conversations, "
                "answer questions, help with tasks, and remember context from the conversation."
            )
        })
        
        if use_history:
            history = get_conversation_history(user_id, limit=MAX_CONVERSATION_HISTORY - 1)
        ...
        logger.info("Preparing Groq chat", user_id=user_id, total_messages=len(messages), history_enabled=use_history)
        ...
        if use_history:
            ...
            save_conversation_message(user_id, "assistant", assistant_message)
        
        return assistant_message
        
    except urllib.error.HTTPError as e:
        error_body = e.read().decode("utf-8", errors="ignore")
        logger.error("Groq API HTTP error", error=e, http_code=e.code, response_body=error_body[:500], 
                     outcome="failure")
        if e.code == 429:
            return "⏳ I'm currently rate-limited. Please try again in a moment."
        elif e.code == 401:
            return "🔒 API authentication failed. Please contact the bot administrator."
        elif e.code == 400:
            return "❌ Invalid request. Your message might be too long or contain unsupported content."
        else:
            return f"❌ API error (HTTP {e.code}). Please try again later."

    except Exception as e:
        logger.error("Groq chat failed", error=e, user_id=user_id, outcome="failure")
...
def ask_ai(question: str) -> str:
    """Legacy function - prefers Groq, falls back to OpenAI, then Gemini"""
    if GROQ_API_KEY:
        try:
            return chat_with_groq(user_id=0, user_message=question, use_history=False)
        except Exception as e:
            logger.warning("Groq failed, trying fallback", error=e)
    ...
    return "No AI provider is configured.\nPlease set GROQ_API_KEY, OPENAI_API_KEY or GEMINI_API_KEY in Lambda."

def summarize_last_notes(user_id: int, limit: int = 10) -> str:
    """Summarize user's recent notes using AI"""
...
def compute_personal_stats(user_id: int) -> str:
    ...
    conv_history = get_conversation_history(user_id, limit=100)
    conv_messages = len(conv_history)
    ...
    if conv_messages > 0:
        lines.append("")
        lines.append(f"🤖 AI Conversations:")
        lines.append(f"  • Total conversation messages: {conv_messages}")
        lines.append(f"  • Using model: {GROQ_MODEL}")
...
def handle_note_commands(chat_id: int, text: str) -> bool:
    """Handle all note-related commands. Returns True if command was handled."""
    ...
    if text.startswith("/updatenote"):
        ...
        # PART 3 - Final continuation of handler.py
...
def help_text() -> str:
    ...
    openai_status = "✅ OpenAI" if OPENAI_API_KEY else "❌ OpenAI"
    gemini_status = "✅ Gemini" if GEMINI_API_KEY else "❌ Gemini"
    ...
    return (
        ...
        "/chat <message> - natural conversation with AI\n"
        ...
        # f"🔧 AI Status: {groq_status} | {openai_status} | {gemini_status}\n"
        # f"💬 Model: {GROQ_MODEL if GROQ_API_KEY else 'N/A'}\n"
        # f"🧠 Memory: {'Enabled' if ENABLE_CONVERSATION_MEMORY else 'Disabled'}\n\n"
...
def handle_text_message(chat_id: int, message_id: int, text: str, first_name: str | None):
    ...
    if text.startswith("/start"):
        ...
        # send_message(chat_id, f"Welcome, {username}! 😊\n\n🤖 I'm powered by Groq AI for natural conversations!\n\nJust send me any message and I'll chat with you, or use /help to see all commands.")
        return
    ...
    # /chat - Natural conversation with Groq
    if text.startswith("/chat "):
        logger.info("Chat command received", user_id=chat_id, action="chat")
        message = text[len("/chat "):].strip()
        if not message:
            send_message(chat_id, "Usage: /chat <your message>")
            return
        
        send_chat_action(chat_id, "typing")
        response = chat_with_groq(chat_id, message, use_history=True)
    ...
    if text == "/clearhistory":
        logger.info("Clear history command received", user_id=chat_id, action="clearhistory")
        if clear_conversation_history(chat_id):
            send_message(chat_id, "🧹 Conversation history cleared! Starting fresh.")
        else:
            send_message(chat_id, "❌ Failed to clear history. Please try again.")
        return
    ...
    if not text.startswith("/"):
        ...
        if GROQ_API_KEY:
            logger.info("Using Groq AI for natural conversation", user_id=chat_id)
            send_chat_action(chat_id, "typing")
            response = chat_with_groq(chat_id, text, use_history=True)
            send_message(chat_id, response)
            return
        elif OPENAI_API_KEY:
            logger.info("Groq not available, falling back to OpenAI", user_id=chat_id)
            send_chat_action(chat_id, "typing")
            response = ask_openai(text)
            send_message(chat_id, response)
            return
        elif GEMINI_API_KEY:
            logger.info("Groq/OpenAI not available, falling back to Gemini", user_id=chat_id)
            send_chat_action(chat_id, "typing")
            response = ask_gemini(text)
            send_message(chat_id, response)
            return
        else:
            logger.warning("No AI provider configured", user_id=chat_id)
            send_message(
                chat_id, 
...
def lambda_handler(event, context):
    ...
    - AI conversations → Powered by Groq with conversation memory
    """
    ...
    if "callback_query" in update:
        ...
        try:
            if message_type == "photo":
                ...
                # Handle command or natural conversation
